...
namespace OSMP
{
    ...
    public class NetPacketReferenceController
    {
        ...
        public NetPacketReferenceController( NetworkModelConnection parent, bool isserver )
        {
            this.parent = parent;
        ...
        void DumpSentPacketsAwaitingAck()
        {
            ...
            {
                Console.WriteLine( dictionaryentry.Key.ToString() + " " + dictionaryentry.Value.ToString() );
            }
        }
            ...
            if( (int)DateTime.Now.Subtract( lastackpacketsent ).TotalMilliseconds > AckPacketIntervalSeconds * 1000 )
            {
                ...
                lock( receivedpacketsnotacked )
                {
                    ...
                    for( int i = 0; i < numpacketstoack; i++ )
                    {
                        ...
                        BinaryPacker.WriteValueToBuffer( ackpacketdata, ref nextposition, typeof( short ), packettoack );
